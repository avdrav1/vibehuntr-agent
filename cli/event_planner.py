...
# Import only the specific modules we need, not the whole app package
...
# Service cache per storage directory
_services: Dict[str, EventPlanningService] = {}


def get_service(storage_dir: str = "data") -> EventPlanningService:
    """Get or create the event planning service instance for a storage directory."""
    if storage_dir not in _services:
        ...
        # Create service
        _services[storage_dir] = EventPlanningService(
...
def list_feedback(ctx, event, user):
    ...
    try:
        ...
        if event:
            ...
            feedbacks = service.feedback_repo.list_all()

...
# Launch local dev playground (ARCHIVED - Streamlit version)
# Note: The Streamlit playground has been archived. Use React + FastAPI instead.
# See DEVELOPMENT_SETUP.md for the recommended React + FastAPI setup.
playground:
	@echo "⚠️  WARNING: The Streamlit playground is archived and no longer maintained."
	@echo "   Recommended: Use the React + FastAPI application instead."
	@echo "   See DEVELOPMENT_SETUP.md for setup instructions."
	@echo ""
	@echo "   Starting archived Streamlit playground..."
...
data-ingestion:
	...
	(cd data_ingestion && uv run data_ingestion_pipeline/submit_pipeline.py \
		...
		--region="us-central1" \
		--data-store-id="vibehuntr-agent-datastore" \
		--data-store-region="us" \
		--service-account="vibehuntr-agent-rag@$$PROJECT_ID.iam.gserviceaccount.com" \
		--pipeline-root="gs://$$PROJECT_ID-vibehuntr-agent-rag" \
